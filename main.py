...
def puzzle_2022_15_2(input=None):
    ...
    for i, (sen, diff) in enumerate(zip(sensors, differences)):
        print(i, ' / ', differences.size)
        x = np.arange(-diff - 1, diff + 2)
        x.dtype = int
        circumference = np.array(
            [np.concatenate((x, x)), np.concatenate((diff + 1 - np.abs(x), -(diff + 1 - np.abs(x))))])
        circumference = np.expand_dims(circumference, axis=0)
        circumference += sen.reshape(2, -1)
        mask1 = np.all((0 < circumference) & (circumference < grid_edge), axis=1)
        mask1 = mask1.flatten()
        mask2 = ~np.any(np.sum(np.abs(sensors.reshape(-1, 2, 1) - circumference), axis=1) <= differences.reshape(-1, 1),
                        axis=0)
        mask2 = mask2.flatten()
        points = circumference[:, :, mask1 & mask2].flatten()
        if (points.size != 0):
